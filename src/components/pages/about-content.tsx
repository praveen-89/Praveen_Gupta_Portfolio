...
export function AboutPageContent() {
  return (
    <>
      ...
      {/* ─── Skills ─── */}
      <Section id="skills" className="relative bg-muted/30 overflow-hidden">
        {/* Decorative background blobs */}
                ...
                >
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center gap-4 mb-3">
                      <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-primary/10 to-accent/10 flex items-center justify-center transform group-hover:scale-110 group-hover:-rotate-3 transition-all duration-300 border border-primary/10 shadow-inner shrink-0">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {cat.skills.map((skill, idx) => (
                        <SkillBadge
                          key={skill}
                          name={skill}
                          delay={catIdx * 0.06 + idx * 0.04}
                        />
                      ))}
                    </div>

                    {cat.usedIn && (
                      <p className="text-[10px] font-semibold uppercase tracking-widest text-primary/60 border-t border-border/50 pt-3">
                        Used in: {cat.usedIn}
                      </p>
                    )}
                  </div>
